			...
			do
			{
				...
				if(pFields[0].GetInt32() == action)
				{
					if(player->GetMoney() < pFields[1].GetUInt32())
					{
							...
							Creature->MonsterWhisper(ss.str().c_str(),(Unit *)player->GetGUID());
					}
					else
					{
...
bool GossipHello_TeleportMaster(Player *player, Creature *Creature)
{
	...
	if(!SD2Config.SetSource(_SCRIPTDEV2_CONFIG))
	{
		...
		Creature->MonsterWhisper("I'm sorry, we are having technical difficulties.  Please check back later.",(Unit *)player->GetGUID());
		return false;
	}
	...
	{
		player->CLOSE_GOSSIP_MENU();
		Creature->MonsterWhisper("You are in combat!",(Unit *)player->GetGUID());
		return false;
	}
	...
	if(SD2Config.GetBoolDefault("TeleportMaster.OnlyGMs", false))
	{
		...
		{
			player->CLOSE_GOSSIP_MENU();
			Creature->MonsterWhisper("Sorry, I am only allowed to teleport game masters.", (Unit *)player->GetGUID());
			return false;
		}
	}
